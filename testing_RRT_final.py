...
class RRT(object):
    ...
    def __init__(self, side1, side2, obs_num, start_point, end_point, obstacle_list):
        """
        """
        self.side_len = side1 + 0.15
        self.side_width = side2 + 0.15
    ...
    def gen_node(self):
        ...
        while not point_ok:
            ...
            node = (parent[1][0] + 0.5*d_x/vec_mag, # change for spacing 
                    parent[1][1] + 0.5*d_y/vec_mag)

            # if newly created node
            print('node')
            print(parent[1], node)
            if self.nodes_list[-1][1] == [0,0]:
                ...
                if self.obstacle_check(node):
                    ...
                    pass
                elif self.path_check(parent[1], node, False):
                    pass
                else:
                    point_ok = True
    ...
    def path_check(self, point, point2, check_end):
        print(point, point2)
        """
    ...
    def gen_tree(self):
        ...
        while not done:
            ...
            if not self.path_check(self.nodes_list[-1][1],self.nodes_list[-1][1], True):
                done = True
...
obs1 = [-0.6148 - 0.1, -0.1 + 0.6767]
obs2 = [-0.275 -0.1 , -0.60919 -0.1]
obs3 = [0.2352 - 0.1, -0.6004 - 0.1]
